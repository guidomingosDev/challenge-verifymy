...
it("Should make the transfer for user 2", () => {
    ...
    cy.login(user1.email, user1.password)
    cy.makeTransfer(account2, digitAccount2, money)
    expect('Transferencia Realizada com Sucesso').to.have.string('Transferencia Realizada com Sucesso');
    cy.closeModal()
    cy.btnBackHome()
    expect(`Saldo em conta R$ ${money}`).to.have.string(money);
    cy.btnExit()
